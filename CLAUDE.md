...
- **Internal linking:** Link to other pages/posts on the same site. Permalinks are often set explicitly in the target file's frontmatter and may NOT match the filename or the slugified title. Always read the target file's `permalink` field before writing any internal link. See the Blog System section for details.
